...
**How to apply:** Enforce expiry before balance display, client balance listings, and point redemption. The public booking page uses the configured 7-, 15-, or 30-day warning threshold for positive balances. Keep the configuration embedded in the existing settings loyalty object unless the product later requires per-earning-date expiration.